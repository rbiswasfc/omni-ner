...
def get_optimizer(cfg, model, print_fn=None):
    ...
    head_layer_name = "loss_fn"  # arcface head
    ...
    optim_groups = [
        ...
        {'params': body_params_decay, 'lr': cfg.optimizer.lr, 'weight_decay': cfg.optimizer.weight_decay},
    ]

    if print_fn is not None:
        n_head_params_no_decay = to_millions(sum(p.numel() for p in head_params_no_decay))
        n_head_params_decay = to_millions(sum(p.numel() for p in head_params_decay))
        n_body_params_no_decay = to_millions(sum(p.numel() for p in body_params_no_decay))
        n_body_params_decay = to_millions(sum(p.numel() for p in body_params_decay))
